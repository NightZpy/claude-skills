...
def save_data(base_dir, data, date=None):
    backup_path = backup_data(base_dir)
    data["meta"]["updated"] = date or today()
    path = os.path.join(base_dir, DATA_FILE)
...
def cmd_add_issue(args, base_dir):
    ...
    node = {
        ...
        "label": args.id,
        "sub": args.sub,
        "url": "https://linear.app/connectly/issue/" + args.id,
    }
...
def cmd_validate(args, base_dir):
    ...
    for n in nodes:
        ...
        if n.get("owner") not in ("lenyn", "other"):
...
def build_parser():
    ...
    sp.add_argument("--id", required=True, help='"iss-AG-1234" or bare "AG-1234"')
    ...
    sp.add_argument("--id", required=True, help="e.g. AG-1234 (without iss- prefix)")
    sp.add_argument("--parent", required=True, help="theme node id")
    sp.add_argument("--sub", required=True)
    sp.add_argument("--owner", choices=["lenyn", "other"], default=None)
