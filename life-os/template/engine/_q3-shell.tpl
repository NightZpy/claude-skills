<title>Employee OS — Lenyn @ Connectly</title>
<style>
  :root{
...
<div class="app">
  <div class="topbar">
    <div class="tb-l"><span class="tb-logo">🕸️</span><div><div class="tb-eyebrow">Q3 2026 en foco · historia H1 incluida</div><div class="tb-t">Employee OS — <em>Lenyn @ Connectly</em></div><div class="tb-s"><!--@TBS--></div></div></div>
...
const GROUPS={
 ...
 lenyn:{fill:"#ecfeff",stroke:"#a5f3fc",text:"#155e75",label:"Lenyn"},
...
const SECTOR_ORDER=["builder","bets","external","people","lenyn","code","shipped","process","cluster"];
const SECTOR_SPAN={builder:42,bets:42,external:23,people:48,lenyn:22,code:20,shipped:42,process:48,cluster:73};
...
NODES.forEach(function(n){
 ...
 var core;
 if(isSat){
  var gr2=grFor(n),isDone=n.state==="done",isTopic=n.kind==="topic",ownerGold=n.owner==="lenyn";
  ...
  var isBet=n.group==="bets",isExt=n.group==="external",isPersonal=n.group==="lenyn",isAlly=n.group==="people";
...
function renderPanel(){
 if(!state.selected){
  ...
  h+="</div><div class='sect'><div class='t'>Empieza por</div><div class='insbtns'><button class='insbtn' data-nav='n-plus-1'>⚡ N+1</button><button class='insbtn' data-nav='persona-library'>📚 Persona Library</button><button class='insbtn' data-nav='m2-loop'>🤖 AG-1716</button><button class='insbtn' data-nav='agent-factory'>🏭 Agent Factory</button></div></div>";
  detBody.innerHTML=h;
  ...
  var n=byId[state.selected],parent=byId[n.parent],gr=grFor(n),isDone=n.state==="done",isMine=n.owner==="lenyn";
  ...
  if(n.lenyn>=2)h+=chipHtml("⚡ palanca Lenyn "+"●".repeat(n.lenyn)+"○".repeat(3-n.lenyn),"#ecfeff","#155e75");
  h+="</div>";
...
function renderHoy(){
 ...
 if(day===0||day===6){
  ...
  var todayHtml="";
  todayHtml+=pushRow(dk+"daily","🌅","Daily en #team-agents + barrido de #agent-failures","r-daily");
  if(day===1)todayHtml+=pushRow(dk+"weekly","🧭","Leads weekly: 3 bullets + actualizar esta guía al salir","r-weekly");
  if(day===3)todayHtml+=pushRow(dk+"review","🏛️","Eng design review (pregunta preparada / presentar)","r-review");
  if(day===5)todayHtml+=pushRow(dk+"health","📊","Publicar la nota de salud de alertas","r-health");
 ...
 var weekHtml="";
 WEEK_PENDINGS.forEach(function(W){weekHtml+=pushRow(wk+W[0],W[1],W[2],W[3]||null);});
 ...
 h+="<button class='ddbtn' data-copytpl='tpl-daily' title='Daily / standup (#team-agents)' style='margin:0 6px 6px 0;font-size:11px;padding:4px 10px'>📋 Daily update</button>";
 ...
 h+="<button class='ddbtn' data-copytpl='tpl-health' title='Nota de salud de alertas' style='margin:0 6px 6px 0;font-size:11px;padding:4px 10px'>📋 Nota de salud</button>";
 h+="</div>";
 box.innerHTML=h;
