...
echo "DISCO: $(echo "$DISK" | awk '{print $3" usados / "$2" ("$4" libres, "$5")"}')"
...
if [ "${1:-}" = "--disk" ]; then
  ...
  {
    echo "== reporte de disco $(date '+%Y-%m-%d %H:%M') =="
    echo "$DISK"
    echo "-- carpetas >1GB (2 niveles bajo \$HOME; padres e hijos se listan por separado)"
    du -xk -d 2 "$HOME" 2>/dev/null | awk '$1>1048576' | sort -rn | head -40 \
      | awk '{s=$1; $1=""; printf "  %7.1fGB %s\n", s/1048576, substr($0,2)}'
    echo "-- archivos sueltos >1GB (tamaño en disco; los de 0B son placeholders de la nube y se omiten)"
...
# --- Notas de esta máquina (fuera del repo: el skill es genérico) ---
[ -f ~/.claude/mac-doctor-notes.md ] && echo "NOTAS: hay notas locales de esta máquina en ~/.claude/mac-doctor-notes.md — léelas antes de proponer acciones"
